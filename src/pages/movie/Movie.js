...
const Movie = () => {
    ...
    useEffect(() => {
        const fetchMovieData = async () => {
            console.log(page)
            const result = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=72f75a773ed8f6ecafa0d7ccc88e68f9&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_genres=${genraURL}`)
    ...
    return (
        <div className="">
            <h1 className='text-center text-capitalize py-3'>Movies</h1>
            <Container>
                <Genra
                    genra={genra}
                    setgenra={setgenra}
